<template>
  <b-sidebar
    ...
    no-header
    right
    @hidden="resetForm; employeeData = null"
  ...
  setup(props, { emit }) {
    ...
    watch(() => props.employee, selection => {
      employeeData.value = {
        ...
        base_salary: selection.base_salary ?? '',
    ...
    const onSubmit = () => {
      store.dispatch('app-employee/updateEmployee', employeeData.value)
        .then(() => {
          emit('refetch-data')
          emit('update:is-edit-employee-sidebar-active', false)
        })
    }
